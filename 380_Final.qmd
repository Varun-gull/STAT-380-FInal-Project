...
```{r class-balance}
#| fig-cap: "Playoff and non-playoff team counts across all six seasons. The classes are fairly balanced because 16 of 30 NBA teams qualify for the playoffs each season."
#| fig-width: 5
#| fig-height: 3.5
...
Over six seasons, `r sum(nba_clean$Playoff == "Yes")` team-seasons ended in a playoff appearance and `r sum(nba_clean$Playoff == "No")` did not. The NBA sends 16 of 30 teams to the playoffs each year, eight from each conference, so the two classes are fairly balanced, with slightly more playoff teams than non-playoff teams. Because this is a classification problem, we report sensitivity and specificity alongside overall accuracy. Accuracy shows the overall percentage of correct predictions, while sensitivity and specificity show whether the models are doing well for both playoff and non-playoff teams.
...
- **Marlon Dubreuil:** Creating visuals, comparsion, recomendations, conclusion
